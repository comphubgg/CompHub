...
export default function KontoSeite() {
  ...
  const [bereich, setBereich] = useState<Bereich>('stats');
      ...
      <div className="mx-auto max-w-3xl">
        ...
        {bereich === 'stats' && (
          <div className="space-y-6">
            ...
            <section className={kasten}>
              <h2 className={ueberschrift}><T>Mein Turnierweg</T></h2>
              <p className="mb-4 text-[11px] leading-relaxed text-slate-600">
                <T>Dein eigenes Abschneiden, Runde für Runde — aus Epics
                Bestenliste.</T>
              </p>
              <MeinErgebnis epicId={meinEpic} turniere={turniere} />
            </section>

            <section className={kasten}>
              <h2 className={ueberschrift}><T>Spieler nachschlagen</T></h2>
              <p className="mb-4 text-[11px] leading-relaxed text-slate-600">
                <T>Tipp einen Spielernamen und wähle ihn aus der Liste — danach
                stehen seine Werte aus dem Turnierarchiv da. Wen du einmal
                gewählt hast, findest du unter dem Feld wieder.</T>
              </p>
              <div className="relative">
                <input value={suchtext}
                  onChange={(e) => setSuchtext(e.target.value)}
                  onKeyDown={(e) => {
                    // Enter nimmt den ersten Vorschlag. Eine Konto-Id tippt
                    // hier niemand ein - siehe der Kommentar am Verlauf.
                    if (e.key !== 'Enter' || !vorschlaege[0]) return;
                    waehleSpieler(vorschlaege[0].epicId, vorschlaege[0].anzeige);
                  }}
                  placeholder={t('Spielername — etwa Vico')}
                  spellCheck={false}
                  /*
                   * Chrome ignoriert "off" bei Textfeldern und schrieb seinen
                   * gemerkten Wert ueber den gerade gewaehlten Namen - im Feld
                   * stand dann etwas anderes als in der Anzeige darunter.
                   * "new-password" unterdrueckt das zuverlaessig, und der
                   * eigene Name haelt das Feld aus der Vorschlagsliste des
                   * Browsers heraus.
                   */
                  autoComplete="new-password"
                  name="epic-spielername"
                  className={feld} />

                {/*
                  * Die Vorschlaege. Wer wie heisst, ist nicht eindeutig -
                  * auf "vico" passen sechs Konten. Deshalb waehlt der Nutzer,
                  * statt dass die Seite raet; Land und Zahl der Matches
                  * stehen dabei, weil man daran den gesuchten erkennt.
                  */}
                {suchtext.trim().length >= 2 && vorschlaege.length > 0 && (
                  <ul className="absolute z-20 mt-1 w-full overflow-hidden
                                 rounded-lg border border-zinc-800 bg-zinc-950
                                 shadow-xl shadow-black/40">
                    {vorschlaege.map((v) => (
                      <li key={v.epicId}>
                        <button
                          onClick={() => waehleSpieler(v.epicId, v.anzeige)}
                          className="flex w-full items-center gap-2 px-3 py-2
                                     text-left text-sm text-slate-200 transition
                                     hover:bg-zinc-900">
                          {v.land && (
                            // eslint-disable-next-line @next/next/no-img-element
                            <img src={`/flags/${v.land.toLowerCase()}.png`} alt=""
                ...
                {suchtext.trim().length >= 2 && !suchtLaeuft
                  && vorschlaege.length === 0
                  && !/^[0-9a-f]{32}$/i.test(suchtext.trim()) && (
                  <p className="mt-2 text-[11px] text-slate-600">
                    <T>Nicht im Archiv — such ihn unten direkt bei Epic.</T>
                  </p>
                )}
              </div>

              {/* Der Verlauf. Hier stand einmal die rohe Konto-Id - sie sagt
                  niemandem etwas und wird ohnehin vom Betreiber vergeben. */}
              {spielerVerlauf.length > 0 && (
                <div className="mt-3 flex flex-wrap items-center gap-1.5">
                  <span className="text-[10px] uppercase tracking-wider
                                   text-slate-600"><T>zuletzt</T></span>
                  {spielerVerlauf.map((e) => (
                    <button key={e.id} onClick={() => waehleSpieler(e.id, e.name)}
                      className={`rounded-full border px-2.5 py-1 text-[11px]
                                  transition ${e.id === epicId
              ...
              {statsFehler && (
                <p className="mt-3 rounded-lg border border-rose-900/60
                              bg-rose-950/30 px-4 py-2.5 text-xs text-rose-300">
                  {statsFehler}
                </p>
              )}
            </section>
            ...
            {/* ------------------------------------------ Suche bei Epic */}
            <section className={kasten}>
              <h2 className={ueberschrift}><T>Direkt bei Epic suchen</T></h2>
              <p className="mb-4 text-[11px] leading-relaxed text-slate-600">
                <T>Das Archiv kennt nur die Profiszene. Wer dort fehlt, steht
                trotzdem im Leaderboard seines Cups — such ihn hier. Ein Cup
                zur Zeit, weil Epic die Bestenlisten je Turnier führt.</T>
              </p>

              <div className="flex flex-wrap gap-2">
                <select value={epicFenster}
                  onChange={(e) => setEpicFenster(e.target.value)}
                    ...
                    <optgroup key={g.id} label={g.titel}>
                      {g.fenster.map((f) => (
                        <option key={f.windowId} value={f.windowId}>
                          {f.region} · {rundenName(f.windowId,
                            /Final/i.test(f.windowId), t) || f.name}
                          {f.ende ? ` · ${new Date(f.ende)
                ...
                <button onClick={epicSuchen}
                  disabled={!epicFenster || !suchtext.trim() || epicLaeuft}
                  className="rounded-lg bg-sky-500 px-5 text-sm font-semibold
                             text-white transition hover:bg-sky-400
                             disabled:cursor-not-allowed disabled:opacity-40">
                  {epicLaeuft ? <T>sucht …</T> : <T>Im Cup suchen</T>}
                </button>
              </div>
                  ...
                  <div className="flex items-center gap-3 px-4 py-3">
                    <span className="relative flex h-2.5 w-2.5 shrink-0">
                      <span className="absolute inline-flex h-full w-full
                                       animate-ping rounded-full bg-sky-500
                                       opacity-75" />
                      <span className="relative inline-flex h-2.5 w-2.5
                                       rounded-full bg-sky-500" />
                    </span>
                    <p className="text-[11px] leading-relaxed text-slate-400">
                      <T>Epic lässt keinen Direktabruf zu — die Bestenliste wird
                      Seite für Seite durchsucht. Das dauert einen Moment.</T>
                    </p>
                  </div>
                  {/* Derselbe laufende Balken wie beim Seitenwechsel. */}
                  <div className="h-0.5 w-full overflow-hidden bg-zinc-900">
                    <div className="h-full w-1/3 bg-sky-500
                                    animate-[comphub-lauf_1.1s_ease-in-out_infinite]" />
                  </div>
                </div>
              )}

              {epicStand && !epicLaeuft && (
                <p className="mt-3 rounded-lg border border-zinc-800
                              bg-zinc-950/60 px-4 py-2.5 text-xs text-slate-400">
                  {epicStand}
                </p>
              )}

              {epicTreffer.length > 0 && (
                <div className="mt-4 space-y-3">
                  {epicTreffer.map((e, i) => {
                    // Epic nennt hier nur Name und Id - alle Zahlen des
                    // Eintrags gehoeren dem Team, nicht der einzelnen Person.
                    const spieler = (e.players ?? []) as Array<{
                      name: string; id?: string;
                    }>;

                    /*
                     * Der Kopf gilt fuer das Team - Platz und Punkte gibt es
                     * nur einmal. Was darunter steht, ist je Spieler.
                     */
                    const team: Array<[string, string]> = [
                      ['Platz', `#${e.rank}`],
                      ['Punkte', String(e.points ?? '—')],
                      ['Runden', String(e.games ?? '—')],
                      ['Siege', String(e.wins ?? '—')],
                      ['Bester Platz', e.bestPlace ? `#${e.bestPlace}` : '—'],
                    ];

                    /*
                     * Die Zeilen der Gegenueberstellung - aus dem Archiv, nicht
                     * von Epic. Nachgeschlagen wird ueber die Konto-Id, nie
                     * ueber den Namen.
                     */
                    const zeilen: Array<[string, string]> = [
                      ...
                      ['Assists', 'assists'],
                      ['Mats Farmed', 'mats'],
                      ['Builds Placed', 'builds'],
                      ['Time Alive', 'timeAlive'],
                    ];
                    const hatEinzeln = spieler.some((p) => p.id && einzeln[p.id]);
                    ...
                    return (
                      <div key={i}
                          ...
                          {team.map(([titel, wert]) => (
                            <div key={titel} className="text-center">
                              <p className={`text-sm font-bold tabular-nums ${
                                wert === '—' ? 'text-slate-700' : 'text-sky-400'}`}>
                                {wert}
                              </p>
                              <p className="mt-0.5 text-[9px] uppercase
                        ...
                        {!hatEinzeln && (
                          <p className="rounded-lg border border-zinc-800
                                        bg-zinc-950/80 px-3 py-2.5 text-[11px]
                                        leading-relaxed text-slate-500">
                            <T>Epic gibt nur Teamwerte heraus. Die Zahlen je
                            Spieler stehen im Archiv — dieser Spieltag liegt
                            dort nicht.</T>
                          </p>
                        )}

                        {/* Die Spieler nebeneinander, Zeile fuer Zeile */}
                        {hatEinzeln && (
                        <div className="overflow-x-auto">
                          <table className="w-full min-w-[380px] text-sm">
                            <thead>
                              <tr className="border-b border-zinc-800">
                                <th className="py-2 text-left text-[10px] font-semibold
                            ...
                            <tbody>
                              {zeilen.map(([titel, feld]) => (
                                <tr key={titel} className="border-b border-zinc-900/60">
                                  <td className="py-1.5 text-[11px] text-slate-500">
                                    <T>{titel}</T>
                                  </td>
                                  {spieler.map((p) => {
                                    const w = p.id ? einzeln[p.id]?.[feld] : undefined;
                                    const v = feld === 'timeAlive'
                                      ? (typeof w === 'number' && w
                                        ? `${Math.floor(w / 3600)}h ${Math.round((w % 3600) / 60)}m`
                                        : '—')
                                      : feld === 'genauigkeit'
                                        ? (typeof w === 'number' ? `${w.toFixed(1)} %` : '—')
                                        : zahlOderStrich(w, true);
                                    return (
                                      <td key={(p.id ?? p.name) + titel}
                                        className={`py-1.5 text-right tabular-nums ${
                                          v === '—' ? 'text-slate-700' : 'text-slate-200'}`}>
                                        {v}
                                      </td>
                                    );
                                  })}
            ...
            {spieler && !statsLaedt && (
              <section className={kasten}>
                <div className="mb-4 flex flex-wrap items-center gap-3">
                  <h2 className={`${ueberschrift} mb-0`}>
                    {String(spieler.anzeige || spieler.name || '')}
                  </h2>
                  <select value={cup} onChange={(e) => setCup(e.target.value)}
                    className="ml-auto rounded-lg border border-zinc-800 bg-zinc-950
                               px-3 py-1.5 text-xs text-slate-200 outline-none
                               focus:border-sky-500">
                    <option value="alle">{t('alle Cups')}</option>
                    {cups.map((c) => (
                      <option key={c.roh} value={c.roh}>
                        {c.titel} ({c.anzahl})
                      </option>
                    ))}
                  </select>
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                  {KACHELN.map(([schluessel, titel]) => {
                    const v = werte?.[schluessel];
                    /*
                     * Ein Strich, wo die Quelle je Cup nichts liefert -
                     * Material und Bauteile stehen nur in der Gesamtsumme.
                     * Eine Null waere dort schlicht falsch.
                     */
                    const fehlt = v === undefined || v === null;
                    return (
                      <div key={schluessel}
                        className="rounded-lg border border-zinc-800 bg-zinc-950/60
                                   px-3 py-3 text-center">
                        <p className={`text-lg font-bold tabular-nums ${fehlt
                          ? 'text-slate-700' : 'text-sky-400'}`}>
                          {fehlt ? '—' : Math.round(Number(v)).toLocaleString('de-DE')}
                        </p>
                        <p className="mt-0.5 text-[10px] uppercase tracking-wider
                                      text-slate-600"><T>{titel}</T></p>
                      </div>
                    );
                  })}
                </div>
                ...
                {cup !== 'alle' && (
                  <p className="mt-3 text-[10px] text-slate-600">
                    <T>Über die Spieltage dieses Cups gerechnet. Material und
                    Bauteile liefert die Quelle nur als Gesamtsumme.</T>
                  </p>
                )}
              </section>
            )}
