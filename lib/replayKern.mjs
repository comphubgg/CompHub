 ...
 *
 * Epic verwirft ein Token gelegentlich vor Ablauf seiner Frist. Bleibt es
 * dann im Speicher stehen, scheitert jede Abfrage bis zu zwei Stunden lang
 * mit 401, obwohl die Zugangsdaten in Ordnung sind.
 */
export function verwirfNutzerToken() { nutzerToken = null; nutzerBis = 0; }
...
export async function matchIds(eventId, windowId, maxSeiten = 10) {
  let { token, accountId } = await holeNutzerToken();
  ...
  for (let seite = 0; seite < maxSeiten; seite++) {
    ...
    /*
     * Ein 401 heisst nicht, dass die Anmeldung weg ist.
     *
     * Epic verwirft ein Token gelegentlich vor Ablauf seiner Frist. Bleibt
     * es im Speicher stehen, scheitert jede weitere Abfrage bis zu zwei
     * Stunden lang - bei einem Lauf, der stundenlang arbeitet, ist das der
     * Unterschied zwischen "alles geholt" und "nichts geholt".
     */
    if (antwort.status === 401) {
      verwirfNutzerToken();
      ({ token, accountId } = await holeNutzerToken());
      antwort = await hole();
    }
    if (!antwort.ok) {
...
/**
 * Den Zustand eines Fensters aus den abgelegten Auswertungen erschliessen.
 *
 * Zwei Fenster im Archiv haben keine _zustand.json mehr, aber zweihundert-
 * neunzig und sechshundertvier fertige Auswertungen liegen daneben. In der
 * Uebersicht fehlten sie deshalb vollstaendig: die Liste ueberging jedes
 * Fenster ohne Verwaltungsdatei stillschweigend, und knapp neunhundert
 * ausgewertete Matches waren nirgends zu sehen. Die Zahlen selbst waren nie
 * weg - die Aggregation liest die Dateien und nicht den Zustand.
 *
 * Erfunden wird nichts: Saison, Fenster, Region, Titel und Zeitpunkt stehen
 * in jeder einzelnen Auswertung. Gelesen wird nur eine davon - die uebrigen
 * sechshundert aufzuschlagen, um dieselbe Antwort zu bekommen, waere
 * Verschwendung. Mit `vollstaendig` werden alle gelesen; das ist fuer den
 * Sammler gedacht, der die Datei danach richtig schreibt.
 */
export async function zustandAusOrdner(season, windowId, vollstaendig = false) {
  ...
  const zustand = { season, windowId, matches: {}, erschlossen: true };

  const uebernehmen = (j) => {
    zustand.eventId ??= j.eventId;
    zustand.region ??= j.region;
    zustand.titel ??= j.titel;
    const t = j.zeitpunkt ? Date.parse(j.zeitpunkt) : 0;
    if (t && t > (zustand.datum ?? 0)) zustand.datum = t;
  };

  if (vollstaendig) {
    for (const d of dateien) {
      let j = null;
      try { j = JSON.parse(await fs.readFile(path.join(ordner, d), 'utf8')); }
      catch { continue; }
      uebernehmen(j);
      zustand.matches[d.slice(0, -5)] = {
        stand: ZUSTAND.FERTIG,
        zeitpunkt: j.zeitpunkt ?? null,
        elims: j.elims?.length ?? 0,
        konten: j.konten?.length ?? 0,
        bytes: j.bytes,
        parserVersion: j.parserVersion,
        pfad: path.relative(process.cwd(), path.join(ordner, d)),
        fehler: null,
      };
    }
    return Object.keys(zustand.matches).length ? zustand : null;
  }

  try {
    uebernehmen(JSON.parse(await fs.readFile(path.join(ordner, dateien[0]), 'utf8')));
  } catch { /* dann eben ohne Titel - die Zahl stimmt trotzdem */ }
  for (const d of dateien) zustand.matches[d.slice(0, -5)] = { stand: ZUSTAND.FERTIG };
  return zustand;
}
...
/* ------------------------------------------------- Die tiefe Auswertung */

/*
 * Was der C#-Leser kann und der Javascript-Leser nicht.
 *
 * fortnite-replay-parser ist seit Juni 2023 unveraendert und bricht auf
 * Fortnite 42.00 ab, sobald man den Netzwerk-Stream lesen laesst
 * ("Too much was read expected 15842"). Nachgeprueft an einem echten
 * Turniermatch, nicht angenommen.
 *
 * fortnite-replay-analysis bringt einen eigenstaendigen C#-Leser mit -
 * FortniteReplayReader, als fertige Binaerdatei fuer win-x64 und linux-x64,
 * also fuer den Laptop und fuer den Laeufer bei GitHub gleichermassen. Er
 * liest dasselbe Match in 2,1 Sekunden.
 *
 * ---------------------------------------------------------------- Grenze
 *
 * Auch damit gibt es KEINEN Schaden, KEINE Trefferquote, KEIN Material und
 * KEINE Bauzeiten. In der Antwort stehen "Stats: null" und
 * "TeamStats: null" - Epic legt diese Werte in den Server-Replays eines
 * Turniers gar nicht erst ab; sie stecken nur in Replays, die ein Spieler
 * selbst aufzeichnet. Das ist keine Frage des Lesers, sondern der Quelle,
 * und es wird hier weder gefuellt noch geschaetzt.
 *
 * Was neu dazukommt, ist dafuer genau das, woran es bisher fehlte: die
 * vollstaendige Lobby mit echter Platzierung. Nicht abgeleitet, nicht aus
 * der Reihenfolge der Eliminierungen geraten - abgelesen.
 */

/** Wie viel eines Replays geholt wird, wenn es tief gelesen werden soll. */
const TIEF_ALLES = 100000;

/**
 * Ein Match tief auswerten: Lobby, Platzierungen, Werte je Spieler.
 *
 * Kostet deutlich mehr als die Ereignisse allein - gemessen 61 MB und rund
 * zwei Sekunden Ladezeit je Match, dazu zwei Sekunden Auswertung. Deshalb
 * gilt es nur fuer Finals; eine offene Runde mit hunderten Lobbys waere
 * damit stundenlang beschaeftigt, und dort begrenzt ohnehin die
 * Zehntausend-Grenze der Bestenliste die Anzeige.
 *
 * Das Replay wandert dafuer kurz auf die Platte: der C#-Leser nimmt einen
 * Dateipfad, keinen Puffer. Die Datei wird danach wieder geloescht -
 * einundsechzig Megabyte je Match waeren sonst nach einem Fenster drei
 * Gigabyte fuer nichts.
 */
export async function leseMatchTief(matchId) {
  const { tmpdir } = await import('os');
  const datei = path.join(tmpdir(), `comphub-${matchId}.replay`);

  let ergebnis = null;
  try {
    const puffer = await downloadReplay({
      matchId,
      dataCount: TIEF_ALLES,
      checkpointCount: TIEF_ALLES,
      eventCount: TIEF_ALLES,
    });
    await fs.writeFile(datei, puffer);

    const { leseReplay } = await import('./replayLeser.mjs');
    ergebnis = await leseReplay(datei);
  } finally {
  ...
  /*
   * Die Aufstellung der Lobby - aus dem, was am Spieler steht.
   *
   * Jeder Spieler traegt seinen Platz und die Nummer seines Teams. Mehr
   * braucht es nicht: Teams entstehen durch Gruppieren, der Platz eines
   * Teams ist der seiner Spieler. Nichts davon ist gerechnet, alles
   * abgelesen - wo Epic keinen Platz eingetragen hat, bleibt er leer.
   */
  const roh = (ergebnis.PlayerData ?? []).filter((p) => p?.EpicId && !p.IsBot);

  const nachTeam = new Map();
  for (const p of roh) {
    const nr = p.TeamIndex ?? null;
    if (nr === null) continue;
    if (!nachTeam.has(nr)) nachTeam.set(nr, { teamIndex: nr, platz: null, spieler: [] });
    const t = nachTeam.get(nr);
    if (t.platz === null && typeof p.Placement === 'number') t.platz = p.Placement;
    t.spieler.push({
      id: String(p.EpicId).toUpperCase(),
      name: p.PlayerName ?? null,
    });
  }
  const teams = [...nachTeam.values()]
    .filter((t) => t.spieler.length)
    .sort((a, b) => (a.platz ?? 9e9) - (b.platz ?? 9e9));

  const jeSpieler = roh.map((p) => ({
    id: String(p.EpicId).toUpperCase(),
    name: p.PlayerName ?? null,
    platz: p.Placement ?? null,
    kills: p.Kills ?? null,
    teamKills: p.TeamKills ?? null,
    /*
     * Wie lange jemand gelebt hat.
     *
     * Der Leser nennt den Todeszeitpunkt in Sekunden seit Rundenbeginn.
     * Wer die Runde ueberlebt hat, hat keinen - dort bleibt es leer,
     * statt die Rundenlaenge als Lebenszeit auszugeben.
     */
    lebenszeit: typeof p.DeathTimeDouble === 'number'
      ? Math.round(p.DeathTimeDouble) : null,
    plattform: p.Platform ?? null,
    verbindungAbgebrochen: Boolean(p.Disconnected),
  }));

  return { teams, spieler: jeSpieler, leser: 'CSharp' };
}
