...
export async function leseMatchTief(matchId) {
  ...
  try {
    ...
    ergebnis = await ReplayAnalysis(datei, { bot: false, sort: true });
  } finally {
  ...
  /*
   * Die Aufstellung der Lobby.
   *
   * "raw" ist das, was im Replay steht. Der Leser bietet daneben noch
   * "generated" und "hybrid" an - errechnete Fassungen fuer Faelle, in
   * denen das Replay unvollstaendig ist. Genommen wird das Abgelesene;
   * wo es fehlt, bleibt der Platz leer statt gerechnet zu werden.
   */
  const platz = ergebnis.processedPlacementInfo?.raw;
  const teams = (platz?.teams ?? []).map((t) => ({
    teamIndex: t.teamIndex ?? null,
    platz: t.placement ?? null,
    spieler: (t.players ?? []).map((p) => ({
      id: (p.epicId ?? '').toUpperCase(),
      name: p.epicName ?? null,
    })).filter((p) => p.id),
  })).filter((t) => t.spieler.length);

  const jeSpieler = (ergebnis.processedPlayerInfo?.raw ?? []).map((p) => ({
    id: (p.EpicId ?? '').toUpperCase(),
    name: p.PlayerName ?? null,
    platz: p.Placement ?? null,
    kills: p.Kills ?? null,
    teamKills: p.TeamKills ?? null,
    // Sekunden, nicht Millisekunden - der Leser gibt sie als Zeichenkette.
    lebenszeit: p.aliveTime === null || p.aliveTime === undefined
      ? null : Math.round(Number(p.aliveTime)),
    plattform: p.Platform ?? null,
    verbindungAbgebrochen: Boolean(p.IsDisconnection),
  })).filter((p) => p.id);
